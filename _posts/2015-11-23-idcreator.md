...
5.生成id的使用
     算法和snowflake之间的比较
     生成的id和db的自增、guid、uuid，自定义string的比较
     我们使用id的方法
...
我们的id生成器先是被设计成用来为我们的内容中心提供服务。对于像我们这种体量的网站，分库分表
是必须要干的事情。我们的书、章、章节都是大数据使用的一个典型。这个大数据还是频繁的业务数据，
并不是传统意义上得类似于日志等数据。它在存储和使用的过程中，必须要做到快速的事务处理。所以
这就给我们提出了一个非常苛刻的要求：最好在服务扩容的时候不要去移动以前的数据。对于这个要求，
我们必须要在数据对象的id上进行一序列的自我定制，以满足我们的需求。  

所以我们设计了一套我们自定义的id生成器算法。在这个自定义的id生成器上，我们除了使用传统的范文模型以外，
我们还要兼顾一些我们自己的特殊需求。我们的业务服务由php、java所组成，所以为了我们系统的简单，
我们对我们的id生成器内置了http服务。以满足不同语言对于同一个id服务的需求。  

但是我们的id生成器也具备传统的分布式系统的一些特性：数据的完整性、一致性、数据的恢复、可靠性、
可用性。同时，为了追求id生成器尽可能的在一个合理的范围内提供服务，我们也考虑了id生成器的成本。
所有的id服务器都构建在传统的、廉价的普通pc服务器上。  

首先，期初我们的设计是id并没有强制的状态。但是因为后来的业务需求，我们又增加了状态的一致性。
因为增加了状态的一致性，所以我们不可避免的，id生成器之间具有分组功能，也有并不是那么复杂的通讯来
确保每次对于id生成器访问的唯一性。但是，因为业务状态的需求，还会有牵连到数据的恢复等一序列的问题。
虽然因为id生成器业务本身的简单性，所以它不需要设计的和DFS一个级别，但是该有的功能也是一个
都不能少得。  

其次，相对于DFS，id生成器的QPS将会大大提高。因为几乎每一次的业务写操作都需要至少一次访问id生成器。
相对于我们的访问量，再按照28原则计算，id生成器的QPS应该至少在5k左右，并且因为id生成器只是业务系统
的一个组件。所以它的响应应该保证在ms级别。如果是到秒，这是绝对不能被接受的。  

第三，id生成器应该被设计成可以方便的扩展。这不仅仅是我们的id生成器应该具备的要求，也是几乎所有的分布式
系统应该具备的要求。在一般的情况下，我们的id生成器被设计成无状态的服务。只有在很少的一小部分需求的情况下，
我们的id生成器才需要被加上“组”的标签，以此来达到一个数据备份和一致性的要求。  

以上这些基本上能都是对id生成器的整个服务来考量。对于我们的id，我们在经过讨论后明确下来以下的几个目标：  

1. 唯一性。每个从id生成器生成的id必须是全系统唯一的。如果在明确业务类型、或者是明确业务系统的模式下，
可以适当的做到对于这个业务唯一即可。这属于id生成器的业务定制部分，必须给与支持；  
2. 短，尽量的简短。虽然现在网络速度和以前相比已经有了长足的进步。但是对于一个日PV在千万以上的系统来说，
每个id节省1b的空间，那么集合页面、Database、RPC的传输，一天大概节省的传输是27G左右。这是一个非常可观的数据；  
3. 运算足够简单。运算并不仅仅是指简单的id间的对比操作，还有很多id的拆分等等的运算。id的拆分基本上都是被用来
做系统的数据路由之用，对于分布式业务系统来说，数据路由是必备并且经常被用到的。在分布式业务系统中，是一个
非常非常基本的运算。所以要原生的支持；  
4. 


而且自从twitter开源了snowflake算法后，几乎需要使用
id生成器的业务基本上都在使用snowflake，
